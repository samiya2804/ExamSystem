  ...
  const examData: ExamData[] = await Promise.all(
    exams.map(async (exam) => {
      ...
      if (user && typeof user === "object" && "email" in user) {
        const faculty = await Faculty.findOne({ email: (user as { email?: string }).email }).lean() as { name?: string } | null;
      ...
      return {
        id: (exam._id as string | number | { toString(): string }).toString(), // <-- make sure it's a string
        title: exam.title,
